...
Two middleware functions by default which are are located in the `char.middleware` namespace.
...
See the `:middleware` key of the `app` definition located in the `char.handler` namespace to manage the enabled middleware.
